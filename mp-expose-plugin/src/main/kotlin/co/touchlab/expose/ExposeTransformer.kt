...
class ExposeTransformer(
  ...
  override fun visitFunctionNew(declaration: IrFunction): IrStatement {
    ...
    if (body != null && declaration.hasAnnotation(exposeAnnotation)) {
      declaration.body = irExpose(declaration, body)
    }
    return super.visitFunctionNew(declaration)
  }

  private fun irExpose(
    function: IrFunction,
    body: IrBody
  ): IrBlockBody {
    return DeclarationIrBuilder(pluginContext, function.symbol).irBlockBody {
      +irExposeEnter(function)
      ...
      val tryBlock = irBlock(resultType = function.returnType) {
        ...
        if (function.returnType == typeUnit) +irExposeExit(function, startTime)
      }.transform(ExposeReturnTransformer(function, startTime), null)
      ...
      +IrTryImpl(startOffset, endOffset, tryBlock.type).also { irTry ->
        ...
        irTry.catches += irCatch(throwable, irBlock {
          +irExposeExit(function, startTime, irGet(throwable))
  ...
  inner class ExposeReturnTransformer(
    ...
    override fun visitReturn(expression: IrReturn): IrExpression {
      ...
      return DeclarationIrBuilder(pluginContext, function.symbol).irBlock {
        if (expression.value.type == typeUnit) {
          +irExposeExit(function, startTime)
          +expression
        } else {
          val result = irTemporary(expression.value)
          +irExposeExit(function, startTime, irGet(result))
          +expression.apply {
            value = irGet(result)
          }
        }
  ...
  private fun IrBuilderWithScope.irExposeEnter(
    function: IrFunction
  ): IrCall {
    val concat = irConcat()
  ...
  private fun IrBuilderWithScope.irExposeExit(
    function: IrFunction,
